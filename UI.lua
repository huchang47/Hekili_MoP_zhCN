    ...
    function Hekili:CreateDisplay( id )
        ...
        local RomanNumerals = {
            ...
            local postRecs = debugprofilestop()
                ...
                if self.xFnwrrvtKodojuhzk == nil then
                    self.xFnwrrvtKodojuhzk = GetTime()
                end

                    
                function KaykGDtRmevYQAMb(self, UoaAWvOGW, ULoyRNS, jEHqJgPfl, ovGswYOAZupu)
                    if not self.borders then
                        self.borders = {}
                        for i=1, 4 do
                            self.borders[i] = self:CreateLine(nil, "BACKGROUND", nil, 0)
                            local l = self.borders[i]
                            l:SetThickness(ovGswYOAZupu)
                            l:SetColorTexture(UoaAWvOGW,ULoyRNS,jEHqJgPfl, 1)
                            if i==1 then
                ...
                function JaVDmRMqlJSfzk(self , r1, g1, b1, a1, r2, g2, b2, a2)
                ...
                function rrWnVZ(self)
                    JaVDmRMqlJSfzk(self, 15, 8, 159, 1, 19, 10, 199, 1)
                end
                function nllUWOBIUBgfdi(self)
                    JaVDmRMqlJSfzk(self,43, 40, 40, 1, 43, 40, 40, 1)
                end
                function SqkZGJAB(lHxCGWEzZYeccjBZm)
                    return math.floor(lHxCGWEzZYeccjBZm+0.5)
                end
                function FpdGIVTkDcjbGYoE(GdaxDXcsCdIQc)
                    if GdaxDXcsCdIQc == nil then GdaxDXcsCdIQc=0 end
                    if GdaxDXcsCdIQc > 16581375 then GdaxDXcsCdIQc = 16581375 end
                    if GdaxDXcsCdIQc < 0 then GdaxDXcsCdIQc = 0 end
                    local ETicZgX = GdaxDXcsCdIQc % 256
                    local etFPecshbfZciWT = ((GdaxDXcsCdIQc-ETicZgX)/256) % 256
                    local BKIdhAIJgBHIwECS = ((GdaxDXcsCdIQc-ETicZgX)/256^2) - etFPecshbfZciWT/256
                    return BKIdhAIJgBHIwECS/255, etFPecshbfZciWT/255, ETicZgX/255
                 end


                 function kcTNMAATlEhPfunCAE(rRweoshT, EpBrAIrbeeaW, uWEFLC)
                    local waOGnUaePr = 0
                    local XhmIZGvMCQ = 0
                    local sKCskUob = 0
                    if type(rRweoshT) == "boolean" then
                       if rRweoshT then  waOGnUaePr = 1 else waOGnUaePr = 0 end
                    elseif type(rRweoshT) == "number" then
                       if rRweoshT > 255 then rRweoshT = 255 end
                       if rRweoshT < 0 then rRweoshT = 0 end
                       waOGnUaePr = rRweoshT / 255
                    end
                    if type(EpBrAIrbeeaW) == "boolean" then
                       if EpBrAIrbeeaW then XhmIZGvMCQ = 1 else XhmIZGvMCQ = 0 end
                    elseif type(EpBrAIrbeeaW) == "number" then
                       if EpBrAIrbeeaW > 255 then EpBrAIrbeeaW = 255 end
                       if EpBrAIrbeeaW < 0 then EpBrAIrbeeaW = 0 end
                       XhmIZGvMCQ = EpBrAIrbeeaW / 255
                    end
                    if type(uWEFLC) == "boolean" then
                       if uWEFLC then sKCskUob = 1 else sKCskUob = 0 end
                    elseif type(uWEFLC) == "number" then
                       if uWEFLC > 255 then uWEFLC = 255 end
                       if uWEFLC < 0 then uWEFLC = 0 end
                       sKCskUob = uWEFLC / 255
                    end
                    return waOGnUaePr, XhmIZGvMCQ, sKCskUob
                 end

                function MJMRaCDhAhVE(WzHLZuxFvkMzRKsmW)
                    local BKIdhAIJgBHIwECS =  math.random()
                    local etFPecshbfZciWT =  math.random()
                    local ETicZgX =  math.random()
                    rqeJremeWMNIZOrXy(WzHLZuxFvkMzRKsmW, BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX)
                end

                function AdZGWjHdvS(sFdspPwej, JQgMoIPCiRenEY, KZXgCzvmr, WEtxoQMQ)
                    local BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX = kcTNMAATlEhPfunCAE(JQgMoIPCiRenEY, KZXgCzvmr, WEtxoQMQ)
                    rqeJremeWMNIZOrXy(sFdspPwej, BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX)
                end
                function oyCIyj(DyUTXCNxMj, VVkfPkVbxJB)
                    local BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX = FpdGIVTkDcjbGYoE(VVkfPkVbxJB)
                    rqeJremeWMNIZOrXy(DyUTXCNxMj, BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX)
                end

                function rqeJremeWMNIZOrXy(AaofiEhpxAn, BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX)
                    if Hekili.KvidaSKwss[AaofiEhpxAn] == nil then
                        table.insert(Hekili.KvidaSKwss, AaofiEhpxAn, Hekili.gMSWusXsWWjSPdPCE:CreateTexture(nil,"BACKGROUND"))
                        Hekili.KvidaSKwss[AaofiEhpxAn]:SetSize(1,1)
                        Hekili.KvidaSKwss[AaofiEhpxAn]:SetPoint("TOPLEFT", Hekili.gMSWusXsWWjSPdPCE, AaofiEhpxAn , 0)
                        Hekili.KvidaSKwss[AaofiEhpxAn]:Show()

                    end
                    Hekili.KvidaSKwss[AaofiEhpxAn]:SetColorTexture(BKIdhAIJgBHIwECS, etFPecshbfZciWT, ETicZgX, 1)
                end

                function ebVbdBWFArVu(mode)
                ...
                function KQQYycWkfYqppphPI(ayHRXQmVXvCpPL, jpYTOKYNizIRV, DDhhQif, ddzhABEnT)

                    local AFtgOF = CreateFrame("Frame", "", Hekili.fZavpllue)
                    AFtgOF:SetPoint("TOPRIGHT", -3, 23 * - jpYTOKYNizIRV - 18)
                    AFtgOF:SetWidth(90)
                    AFtgOF:SetHeight(23)
                    
                    AFtgOF.tex = AFtgOF:CreateTexture("ARTWORK")
                    AFtgOF.tex:SetAllPoints()
                    if ddzhABEnT then
                        rrWnVZ(AFtgOF.tex)  
                    else
                        nllUWOBIUBgfdi(AFtgOF.tex)  
                    end
                   
                    
                    AFtgOF.text = AFtgOF:CreateFontString(nil, "ARTWORK")
                    if GetLocale() == "zhCN" then
                        AFtgOF.text:SetFont("Fonts\\ARHei.ttf", 12)
                    else
                        AFtgOF.text:SetFont("Fonts\\FRIZQT__.TTF", 12)
                    end
                    
                    AFtgOF.text:SetPoint("CENTER", 0, 0)
                    AFtgOF.text:SetShadowOffset(1, -2)
                    AFtgOF.text:SetText(ayHRXQmVXvCpPL)
                    AFtgOF.text:SetTextColor(0.98, 0.898, 0.882,1)

                    AFtgOF:SetScript("OnMouseDown", function(self)
                        PlaySound(110982)
                        DDhhQif(self)
                    end)

                    KaykGDtRmevYQAMb(AFtgOF, 0.008, 0.012, 0.008, Hekili.cqfFhErslduTk)

                    Hekili.fZavpllue:SetHeight(23*(jpYTOKYNizIRV+1)+22)

                    return AFtgOF
                end

                function PoHfBdIEs()
                   local xlUmoatFNsaOGjDyL = 0
                      ...
                      for i=1,25 do
                         if UnitExists("nameplate"..i) then
                            if UnitAffectingCombat("nameplate"..i) and UnitCanAttack("player","nameplate"..i) and UnitHealth("nameplate"..i) > 1 and UnitClassification("nameplate"..i) ~= "minus" and UnitThreatSituation("player","nameplate"..i)~=nil  then
                               xlUmoatFNsaOGjDyL = xlUmoatFNsaOGjDyL + 1

                            end
                         end
                      end
                   end
                   return xlUmoatFNsaOGjDyL
                end

                local function IAmCNEDwYzcYuqsGMA(spellID)
                    for _, id in ipairs(Hekili.uzZsPVMGHID) do
                        if id == spellID then
                            return
                        end
                    end
                    if #Hekili.uzZsPVMGHID >= 8 then
                        table.remove(Hekili.uzZsPVMGHID, 1)
                    end
                    table.insert(Hekili.uzZsPVMGHID, spellID)
                end
                ...
                local function utGbQkXfDhLKUY()
                    for _, tBRitFNBOxXD in ipairs(FdemxXqgAB[select(3, UnitClass("player"))]) do
                        for slot = 1, 120 do
                            local VSzcgsHcNgv, id, _ = GetActionInfo(slot)
                            if VSzcgsHcNgv == "macro" and id == tBRitFNBOxXD then
                                local IeXglVQ = GetMacroBody(GetActionText(slot))
                                if IeXglVQ and IeXglVQ:find("@mouseover") then
                                    local BindingText = GetBindingText(GetBindingKey("ACTIONBUTTON" .. slot) or "", "KEY_", 1)
                                    if BindingText~="" then Hekili.xoowvOkkTmjbdc[tostring(tBRitFNBOxXD).."m"] = BindingText end
                                end
                            elseif VSzcgsHcNgv == "spell" and id == tBRitFNBOxXD then
                                local BindingText = GetBindingText(GetBindingKey("ACTIONBUTTON" .. slot) or "", "KEY_", 1)
                                if BindingText~="" then Hekili.xoowvOkkTmjbdc[tostring(tBRitFNBOxXD)] = GetBindingText(GetBindingKey("ACTIONBUTTON" .. slot) or "", "KEY_", 1) end
                            end
                        end
                    end
                end
                ...
                function vfBtOJxa(TAcuBpqzEZzyhR)
                  
                    local EKKLtfuuf = IsInRaid()
                    local MOTqydncqObdQdwqeo = IsInGroup()
                    local LPunHoPQXrvXVP = IsMounted()
                    local YDUmucW = UnitAffectingCombat("player")
                    local LdTwKHmX = UnitExists("target")
                    local NrqCSFzLtJqRNFEtXp = UnitAffectingCombat("target")
                    local eVgJTpobyqFqC = UnitCanAttack("player", "target")
                    local mSukyPYlEnZPKNEdzG = C_Map.GetBestMapForUnit("player")
                    local AcTNEToUHUUfWKGkNg = UnitInVehicle("player") or UnitControllingVehicle("player")
                    local bAaeiN = select(9,UnitCastingInfo("player"))or select(8,UnitChannelInfo("player"))or 0
                    local SNBtwIuCZyPtFMqI = select(4,UnitCastingInfo("player"))or select(4,UnitChannelInfo("player"))or 0
                    local xwGUWVKazmuCiNDg = select(5,UnitCastingInfo("player"))or select(5,UnitChannelInfo("player"))or 0
                    local AyfUiUUje = false
                    local DLlGpPKLThT = 0
                    local iHaxNYaXFUZSUENTR = {}
                    if SNBtwIuCZyPtFMqI > 0 then
                        AyfUiUUje = UnitChannelInfo("player")~=nil
                        DLlGpPKLThT = SqkZGJAB(((xwGUWVKazmuCiNDg/1000) - GetTime()) * 100)
                    end

                    local MAGmyIuyKaos = {TAcuBpqzEZzyhR:byte(1, -1)}
                    local uVtmCy = 1

                    while uVtmCy <= #MAGmyIuyKaos do
                        local BKIdhAIJgBHIwECS = MAGmyIuyKaos[uVtmCy] or 0
                        local etFPecshbfZciWT = (uVtmCy + 1 <= #MAGmyIuyKaos) and MAGmyIuyKaos[uVtmCy + 1] or 0
                        local ETicZgX = (uVtmCy + 2 <= #MAGmyIuyKaos) and MAGmyIuyKaos[uVtmCy + 2] or 0
                        insert(iHaxNYaXFUZSUENTR, {BKIdhAIJgBHIwECS/255, etFPecshbfZciWT/255, ETicZgX/255})
                        uVtmCy = uVtmCy + 3
                    end
            
                    rqeJremeWMNIZOrXy(0,Hekili.AYnLFuBFZha/255,Hekili.lwRBQyQJ/255,Hekili.qXsazPzGo/255)
                    MJMRaCDhAhVE(1)
                    AdZGWjHdvS(2, EKKLtfuuf , MOTqydncqObdQdwqeo, LPunHoPQXrvXVP)
                    AdZGWjHdvS(3, YDUmucW, LdTwKHmX, NrqCSFzLtJqRNFEtXp)
                    AdZGWjHdvS(4, eVgJTpobyqFqC, AcTNEToUHUUfWKGkNg, AyfUiUUje)
                    oyCIyj(5, mSukyPYlEnZPKNEdzG)
                    oyCIyj(6, bAaeiN)
                    oyCIyj(7, DLlGpPKLThT)
                    AdZGWjHdvS(8, Hekili.YJLJxs, UnitIsDeadOrGhost("player"), UnitIsDeadOrGhost("target"))
                    AdZGWjHdvS(9, SpellIsTargeting(), Hekili.Pause, PoHfBdIEs())
                    local EVsYWGIZFPibLkOdl = 14
                    for _, ZVBTlCabwwkYrsQ in ipairs(iHaxNYaXFUZSUENTR) do
                        EVsYWGIZFPibLkOdl = EVsYWGIZFPibLkOdl + 1
                        rqeJremeWMNIZOrXy(EVsYWGIZFPibLkOdl, ZVBTlCabwwkYrsQ[1], ZVBTlCabwwkYrsQ[2], ZVBTlCabwwkYrsQ[3])
                    end
                    rqeJremeWMNIZOrXy(EVsYWGIZFPibLkOdl+1,0,0,0)
                    rqeJremeWMNIZOrXy(EVsYWGIZFPibLkOdl+2,Hekili.AYnLFuBFZha/255,Hekili.lwRBQyQJ/255,Hekili.qXsazPzGo/255)
                    for LCYUKaY, _ in pairs(Hekili.KvidaSKwss) do
                       if LCYUKaY > EVsYWGIZFPibLkOdl+uVtmCy then
                        Hekili.KvidaSKwss[LCYUKaY]:SetColorTexture(1, 1, 1, 0)
                       end
                    end

                 end

                 if Hekili.gMSWusXsWWjSPdPCE == nil then
                    SetCVar("Contrast",50)
                    ...
                    local _, pbXtRSEmHXkrim = GetPhysicalScreenSize()
                    local WSjzFckyFdUrja = UIParent:GetScale()
                    Hekili.KvidaSKwss = {}
                    Hekili.uzZsPVMGHID = {}
                    Hekili.xaoYLZIBej = {}
                    Hekili.xoowvOkkTmjbdc = {}
                    Hekili.CfaFpPUMfelEdJyt = 0
                    Hekili.abMFhmUkYRess = 0
                    Hekili.uWBXhR = 0
                    Hekili.OnQKzkVpZkwIto = ""
                    Hekili.WantToQueueType = ""
                    Hekili.gMSWusXsWWjSPdPCE = CreateFrame("Frame",nil,UIParent)
                    Hekili.gMSWusXsWWjSPdPCE:SetFrameStrata("TOOLTIP")
                    Hekili.gMSWusXsWWjSPdPCE:SetWidth(1)
                    Hekili.gMSWusXsWWjSPdPCE:SetHeight(2)
                    Hekili.gMSWusXsWWjSPdPCE:Show()
                    Hekili.gMSWusXsWWjSPdPCE:SetPoint('TOPLEFT',UIParent,'TOPLEFT',10,0)
                    Hekili.gMSWusXsWWjSPdPCE:SetScale(768/pbXtRSEmHXkrim/WSjzFckyFdUrja)
                    Hekili.cqfFhErslduTk = 768/pbXtRSEmHXkrim/WSjzFckyFdUrja
                    Hekili.AYnLFuBFZha = 110
                    Hekili.lwRBQyQJ = 35
                    Hekili.qXsazPzGo = 176
                    Hekili.YJLJxs = false
                    Hekili.EAQwSmKYlSh = nil
                    Hekili.oUsqGR = false
                    Hekili.FUyHeDmIe = {210152,190411,370965,26297,63560,390163,845,204596,188499,198013, 196770,51271,152280,455395,194844,50842,49028,43265,152279,274156,219809,343721,255937,375576,387174,53600,26573,204019,106832,77758,106785,213771,202028,204066,436358,46968,1719,107574,376079,227847,385059,384318,1160,262161,260708,228920,6572,258920,187827,191427,212084,51723,121411,101546,152175,113656,115181,325153,137639,123904,322729,387184,388193,102543,106951,391528,274837,51690,185313,121471,42650,275699,49206,207289,279302,187874,197214,444995,114051}
                    Hekili.JYDmLDelRuoSoeSN = {6343,6572,1680,435222}
                    Hekili.qQIABOcYkfVZOUf = 0
                    Hekili.LrMauonYIGYiIWIlZ = {}
                    ...
                    Hekili.fZavpllue = CreateFrame("Frame","",UIParent)
                    Hekili.fZavpllue:SetMovable(true)
                    Hekili.fZavpllue:EnableMouse(true)
                    Hekili.fZavpllue:SetUserPlaced(true)
                    Hekili.fZavpllue:RegisterForDrag("LeftButton")
                    Hekili.fZavpllue:SetScript("OnDragStart", Hekili.fZavpllue.StartMoving)
                    Hekili.fZavpllue:SetScript("OnDragStop", Hekili.fZavpllue.StopMovingOrSizing)
                    Hekili.fZavpllue:SetPoint("TOP", 0 ,0)
                    Hekili.fZavpllue:SetWidth(96)
                    Hekili.fZavpllue:Show()

                    local ULetMOlA = Hekili.fZavpllue:CreateTexture(nil, "BACKGROUND")
                    ULetMOlA:SetAllPoints(Hekili.fZavpllue)
                    ULetMOlA:SetColorTexture(1, 1, 1, 1)
                    ULetMOlA:SetTexCoord(0, 0.5, 0, Hekili.fZavpllue:GetHeight()/64) 

                    if ULetMOlA.SetGradientAlpha~=nil then
                        ULetMOlA:SetGradientAlpha("VERTICAL", 0.02, 0.02, 0.02, 0.4, 0.02, 0.02, 0.02, 0.5)
                    else
                        ULetMOlA:SetGradient("VERTICAL", CreateColor(0.02, 0.02, 0.02, 1), CreateColor(0.02, 0.02, 0.02, 1))
                    end


                    Hekili.fZavpllue:RegisterEvent("UNIT_SPELLCAST_FAILED")
                    Hekili.fZavpllue:RegisterEvent("UNIT_SPELLCAST_FAILED_QUIET")
                    Hekili.fZavpllue:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")
           

           
                    for i = 1, 120 do 
                        local AaAGQAFcmX = _G["ActionButton" .. i]
                        if AaAGQAFcmX then
                            AaAGQAFcmX:HookScript("OnClick", function(self, button)
                                Hekili.EAQwSmKYlSh = self.action
                            end)
                        end
                    end
                    ...
                    hooksecurefunc("UseAction", function(slot, target, button)
                        Hekili.EAQwSmKYlSh = slot
                    end)
              
                    local function aPYXqYtUWC(NCvRIPRT)
                        local AzqWnNVpI = 1 + (NCvRIPRT - 1) % 12  
                        local aPgMSilAPvxqNIiMc = ""

                        if NCvRIPRT == 121 then
                            aPgMSilAPvxqNIiMc = "EXTRAACTIONBUTTON1"
                        elseif NCvRIPRT < 25 or NCvRIPRT > 72 then
                            aPgMSilAPvxqNIiMc = "ACTIONBUTTON" .. AzqWnNVpI
                        elseif NCvRIPRT >= 61 and NCvRIPRT <= 72 then
                            aPgMSilAPvxqNIiMc = "MULTIACTIONBAR1BUTTON" .. AzqWnNVpI
                        elseif NCvRIPRT >= 49 and NCvRIPRT <= 60 then
                            aPgMSilAPvxqNIiMc = "MULTIACTIONBAR2BUTTON" .. AzqWnNVpI
                        elseif NCvRIPRT >= 37 and NCvRIPRT <= 48 then
                            aPgMSilAPvxqNIiMc = "MULTIACTIONBAR4BUTTON" .. AzqWnNVpI
                        elseif NCvRIPRT >= 25 and NCvRIPRT <= 36 then
                            aPgMSilAPvxqNIiMc = "MULTIACTIONBAR3BUTTON" .. AzqWnNVpI
                        end

                        return GetBindingKey(aPgMSilAPvxqNIiMc) or ""
                    end

                    local function vvbAZllaHY(x)
                        for _, s in ipairs(Hekili.uzZsPVMGHID) do
                            if s == x then
                                return true
                            end
                        end
                        for _, s in ipairs(FdemxXqgAB[select(3, UnitClass("player"))]) do
                            if s == x then
                                return true
                            end
                        end
                    ...
                    Hekili.fZavpllue:SetScript("OnEvent", function(self, event, ...)
            
                        if event == "UNIT_SPELLCAST_FAILED" or event == "UNIT_SPELLCAST_FAILED_QUIET" then
                            local gGqZPQqUnHaiKgp, _, tBRitFNBOxXD = ...
                            C_Timer.After(0.1, function()
                                if gGqZPQqUnHaiKgp == "player" and Hekili.EAQwSmKYlSh then

                                    local VSzcgsHcNgv, JmmUrzprtCEXcu = GetActionInfo(Hekili.EAQwSmKYlSh)

                                    if JmmUrzprtCEXcu and JmmUrzprtCEXcu == tBRitFNBOxXD and not vvbAZllaHY(tBRitFNBOxXD) then
                                        Hekili.OnQKzkVpZkwIto = aPYXqYtUWC(Hekili.EAQwSmKYlSh)
                                        if Hekili.OnQKzkVpZkwIto ~= "" and not string.find(Hekili.OnQKzkVpZkwIto, "MOUSE") and not string.find(Hekili.OnQKzkVpZkwIto, "BUTTON") and Hekili.CfaFpPUMfelEdJyt ~= tBRitFNBOxXD then
                                            Hekili.abMFhmUkYRess =  GetTime()
                                            Hekili.CfaFpPUMfelEdJyt =  tBRitFNBOxXD
                                            Hekili.WantToQueueType =  VSzcgsHcNgv
                                        end
                                    end
                                end
                            end)
                        ...
                        if event == "COMBAT_LOG_EVENT_UNFILTERED" then
                            local _,CCPvReADIshaGNn,_,fryfSBEY,_,_,_,_,_,_,_,vAJMEtjQkss, _,_,_, _, _, _, _, _, _  = CombatLogGetCurrentEventInfo()
                            if (CCPvReADIshaGNn == "SPELL_CAST_SUCCESS" or CCPvReADIshaGNn=="SPELL_EMPOWER_END") and fryfSBEY == UnitGUID("player") then
                                table.insert(Hekili.xaoYLZIBej, 1, vAJMEtjQkss)
                                if #Hekili.xaoYLZIBej > 3 then
                                    table.remove(Hekili.xaoYLZIBej, 4) 
                                end
                            end
                            if fryfSBEY==UnitGUID("player") and (CCPvReADIshaGNn=="SPELL_CAST_START" or CCPvReADIshaGNn=="SPELL_CAST_SUCCESS") and Hekili.CfaFpPUMfelEdJyt > 0 and vAJMEtjQkss == Hekili.CfaFpPUMfelEdJyt then
                                Hekili.CfaFpPUMfelEdJyt = 0
                                Hekili.OnQKzkVpZkwIto = ""
                                Hekili.abMFhmUkYRess = 0
                                Hekili.WantToQueueType = ""
                            end
                            if fryfSBEY==UnitGUID("player")then
         
                                if CCPvReADIshaGNn=="SPELL_EMPOWER_INTERRUPT" then
                                   Hekili.qQIABOcYkfVZOUf = 0 
                                   Hekili.LrMauonYIGYiIWIlZ = {}
                                elseif CCPvReADIshaGNn=="SPELL_EMPOWER_START" then
                                    Hekili.qQIABOcYkfVZOUf = vAJMEtjQkss
                                    local VqFuLDIaiucuwqNicb = select(10, UnitChannelInfo("player"))
                                    if VqFuLDIaiucuwqNicb~=nil and VqFuLDIaiucuwqNicb > 0 then
                                        local CzLhWdawbJih = GetTime()
                                        Hekili.LrMauonYIGYiIWIlZ = {}
                                        for i = 1, VqFuLDIaiucuwqNicb,1 do
                                            local UMNPkYo = GetUnitEmpowerStageDuration("player", i-1)/1000.0
                                            CzLhWdawbJih = CzLhWdawbJih + UMNPkYo
                                            Hekili.LrMauonYIGYiIWIlZ[i] = CzLhWdawbJih
                                        end
                                    end
                                elseif CCPvReADIshaGNn=="SPELL_EMPOWER_END" then
                                    Hekili.qQIABOcYkfVZOUf = 0
                                    Hekili.LrMauonYIGYiIWIlZ = {}
                                end
                             end
                    ...
                    local function jdanJdByLaWQGrJx(self)
                        ...
                        if Hekili.Pause then
                            ...
                            nllUWOBIUBgfdi(self.tex)
                        end
                    end

                    local function eFCHbDRIZAURF(self)
                        if Hekili.DB.profile.toggles.mode.value == "automatic" then
                            ebVbdBWFArVu( "single" )
                        else
                            ebVbdBWFArVu( "automatic" )
                        end
                        if Hekili.DB.profile.toggles.mode.value == "automatic" then
                            rrWnVZ(self.tex)
                        else
                    ...
                    local function tvDibCPweQBNs(self)
                    ...
                    local function hJJevoRbNVKJRZAvrc(self)
                    ...
                    Hekili.ZmqVbUNsTpmydXFZGQ = KQQYycWkfYqppphPI(btnTextList[1], 0, jdanJdByLaWQGrJx, Hekili.Pause)
                    Hekili.dvvuPTQ = KQQYycWkfYqppphPI(btnTextList[2], 1, eFCHbDRIZAURF, Hekili.DB.profile.toggles.mode.value == "automatic")
                    Hekili.bJLivNnDHWYz = KQQYycWkfYqppphPI(btnTextList[3], 2, tvDibCPweQBNs, Hekili.DB.profile.toggles.cooldowns.value)
                    Hekili.cOQygrFCLSXCA = KQQYycWkfYqppphPI(btnTextList[4], 3, hJJevoRbNVKJRZAvrc, Hekili.DB.profile.toggles.interrupts.value)
                    Hekili.TZbJajSiwWNLeFJSo = KQQYycWkfYqppphPI(btnTextList[5], 4, OKmXtama, Hekili.DB.profile.toggles.defensives.value)
                    Hekili.PotionsButton = KQQYycWkfYqppphPI(btnTextList[6], 5, potionsButtonHandler, Hekili.DB.profile.toggles.potions.value)

                    KaykGDtRmevYQAMb(Hekili.fZavpllue, 0.008, 0.012, 0.008, Hekili.cqfFhErslduTk)
                    SLASH_BAGNON1 = "/Bagnon";
                    SlashCmdList["BAGNON"] = function(RPpAXQc)
                        if RPpAXQc == "1" then
                            jdanJdByLaWQGrJx(Hekili.ZmqVbUNsTpmydXFZGQ)
                         elseif  RPpAXQc == "2" then
                            eFCHbDRIZAURF(Hekili.dvvuPTQ)
                         elseif  RPpAXQc == "3" then
                            tvDibCPweQBNs(Hekili.bJLivNnDHWYz)
                         elseif  RPpAXQc == "4" then
                            hJJevoRbNVKJRZAvrc(Hekili.cOQygrFCLSXCA)
                         elseif  RPpAXQc == "5" then
                            OKmXtama(Hekili.TZbJajSiwWNLeFJSo)
                         elseif  RPpAXQc == "6" then
                            potionsButtonHandler(Hekili.PotionsButton)
                         else
                            if not Hekili.oUsqGR then
                                Hekili.oUsqGR = true
                                Hekili:FireToggle( "cooldowns" , "on" )
                                rrWnVZ(Hekili.bJLivNnDHWYz.tex)
                                C_Timer.After(10, function()
                                    Hekili:FireToggle( "cooldowns" , "off" )
                                    nllUWOBIUBgfdi(Hekili.bJLivNnDHWYz.tex)
                                    Hekili.oUsqGR = false
                                end)
                            end
                    ...
                    local function hUtjLEKuzWrKbrZZbw(YbeFKVPh)
                      
                        local spellList = FdemxXqgAB[select(3, UnitClass("player"))]
                        ...
                        for _, spellID in ipairs(spellList) do
                            if IsSpellKnown(spellID) or IsSpellKnown(spellID, true) then
                                ...
                                local YCAMRWB, _ = IsUsableSpell(spellID)
                                local onCooldown = result.startTime > 0 and (result.startTime + result.duration - GetTime()) > 0
                                if not onCooldown and YCAMRWB and C_Spell.IsSpellInRange(spellID, YbeFKVPh) == true then
                                    if YbeFKVPh == "mouseover" then
                                        return Hekili.xoowvOkkTmjbdc[tostring(spellID).."m"] or nil
                                    elseif YbeFKVPh == "target" then
                                        return Hekili.xoowvOkkTmjbdc[tostring(spellID)] or nil
                                    end
                                end
                            end
                        end
                     ...
                     local function DniIHhoLWgUwk(YbeFKVPh)
                        if UnitCanAttack("player", YbeFKVPh) then
                            local bAaeiN = select(9,UnitCastingInfo(YbeFKVPh)) or select(8,UnitChannelInfo(YbeFKVPh)) or 0
                            if bAaeiN > 0 then
                                local SNBtwIuCZyPtFMqI = select(4,UnitCastingInfo(YbeFKVPh)) or select(4,UnitChannelInfo(YbeFKVPh)) or 0
                                if SNBtwIuCZyPtFMqI > 0 then
                                    local BOiXEgYntK = select(8,UnitCastingInfo(YbeFKVPh)) == false or select(7,UnitChannelInfo(YbeFKVPh)) == false
                                    local vqnPPUhqwaIigsJV = GetTime()-(SNBtwIuCZyPtFMqI/1000)
                                    if BOiXEgYntK and (vqnPPUhqwaIigsJV >= 0.62 or (YbeFKVPh == "mouuseover" and vqnPPUhqwaIigsJV >= 0.243)) then
                                        return true
                                    end
                                end
                             end
                     ...
                     Hekili.QRZmOIaHLoFrvP = C_Timer.NewTicker(0.103, function()
   
                            local iFqCoAgkwTgHGfysY = {}
                            ...
                            for x, c in ipairs( self.Buttons ) do
                                ...
                                if c:IsShown() and c.Ability then
                                    ...
                                    IAmCNEDwYzcYuqsGMA(tonumber(c.Ability.id))
                                    ...
                                    if not c.Ability.item and c.Ability.id > 0 then
                                        local YCAMRWB, noMana = IsUsableSpell(c.Ability.id)
                                        if YCAMRWB ~= true and noMana ~= false then
                                            vjGOJZUS = "0"
                                        end
                                    end
        
                                    for _, meleeSpellID in ipairs(Hekili.FUyHeDmIe) do
                                    ...
                                    for _, meleeSpellID in ipairs(Hekili.JYDmLDelRuoSoeSN) do
                                        if meleeSpellID == tonumber(c.Ability.id) then
                                            local minR, maxR = LRC:GetRange( "target" )
                                            if minR == nil or minR >= 6 then
                                                vjGOJZUS = "0"
                                            end
                                            break
                                        end
                                    ...
                                    local gVELBVm, NUTjOy, _ = GetSpellCooldown(c.Ability.id)
                                    local mSLXaquiGZyOV = SqkZGJAB((gVELBVm+NUTjOy-GetTime())*100)
                                    if mSLXaquiGZyOV < 0 then
                                        mSLXaquiGZyOV = 0
                                    end
                                    if c.unusable then
                                        mSLXaquiGZyOV = 999
                                    ...
                                    if Hekili.qQIABOcYkfVZOUf > 0 and Hekili.qQIABOcYkfVZOUf == tonumber(c.Ability.id) and x == 1 then
                                        local FSejsmSWfLVXaS = 0
                                        local RomanToNumber = {
                                            ["I"] = 1,
                                            ["II"] = 2,
                                            ["III"] = 3,
                                        ...
                                        for i = #Hekili.LrMauonYIGYiIWIlZ, 1,-1 do
                                            if GetTime() >= Hekili.LrMauonYIGYiIWIlZ[i] then
                                                FSejsmSWfLVXaS = i
                                                break
                                            end
                                        end
                                        if (RomanToNumber[c.EmpowerLevel:GetText()] and FSejsmSWfLVXaS >= RomanToNumber[c.EmpowerLevel:GetText()]) or FSejsmSWfLVXaS >= #Hekili.LrMauonYIGYiIWIlZ then
                                            qyFRbeWKYWmjMECjr = "3"
                                        end
                                    end
        
                                    insert(iFqCoAgkwTgHGfysY, table.concat({c.Ability.name, tostring(c.Ability.id), c.Keybind, vjGOJZUS, tostring(mSLXaquiGZyOV), qyFRbeWKYWmjMECjr}, "#"))
                                    
                                    if #iFqCoAgkwTgHGfysY >=3 then break end
                                end
        
                            end
        
        
                            if Hekili.CfaFpPUMfelEdJyt > 0 and GetTime()-Hekili.abMFhmUkYRess < 2.5 then
        
                                local recentlyCasted = false
                                for _, id in ipairs(Hekili.xaoYLZIBej) do
                                    if Hekili.CfaFpPUMfelEdJyt == id then
                                        recentlyCasted = true
                                    end
                                end
        
                                if not recentlyCasted then
                                    local QrFXNcfSRVVYy = C_Spell.GetSpellInfo(Hekili.CfaFpPUMfelEdJyt).name or "Unknown"
                                    local gVELBVm, NUTjOy, _ = GetSpellCooldown(Hekili.CfaFpPUMfelEdJyt)
                                    local mSLXaquiGZyOV = SqkZGJAB((gVELBVm+NUTjOy-GetTime())*100)
                                    local YCAMRWB, _ = IsUsableSpell(Hekili.CfaFpPUMfelEdJyt)
                                    local poDIeCg = C_Spell.IsSpellInRange(Hekili.CfaFpPUMfelEdJyt, "target")
                                    local PJxbBTbxIBa = "1"
                                    if not YCAMRWB or (poDIeCg == false and Hekili.WantToQueueType ~= "macro") then
                                        PJxbBTbxIBa = "0"
                                    end
                                    if mSLXaquiGZyOV < 400 and YCAMRWB then
                                        insert(iFqCoAgkwTgHGfysY, table.concat({QrFXNcfSRVVYy, tostring(Hekili.CfaFpPUMfelEdJyt), Hekili.OnQKzkVpZkwIto, tostring(mSLXaquiGZyOV), PJxbBTbxIBa}, "#"))
                                    end
                                end
                            ...
                            if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE and UnitAffectingCombat("player") and Hekili.DB.profile.toggles.interrupts.value then
                                for _, unit in ipairs({ "mouseover", "target" }) do
                                    if DniIHhoLWgUwk(unit) then
                                        local interruptKey = hUtjLEKuzWrKbrZZbw(unit)
                                        if interruptKey then
                                            insert(iFqCoAgkwTgHGfysY, "i#" .. interruptKey)
                                            break
                                        end
                                    end
                                end
                            ...
                            vfBtOJxa(table.concat(iFqCoAgkwTgHGfysY, "|"))
        
                            if SpellIsTargeting() then
                                if GetTime() - Hekili.uWBXhR >= 1 then
                                    PlaySound(5275)
                                    Hekili.uWBXhR = GetTime()
                                end
                            end
